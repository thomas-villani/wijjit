...
def main_view():
    ...
    return {
        "template": """
{% frame title="Wijjit Charts Demo" border_style="rounded" width="fill" height="fill" %}
  {% vstack spacing=1 %}

    {# Row 1: Sparklines and Gauges #}
    {% hstack spacing=2 %}
      {% frame title="Sparklines" border_style="single" width=38 height=10 %}
      ...
      {% frame title="Gauges" border_style="single" width=38 height=10 %}
        {% vstack spacing=0 %}
    ...
    {% hstack spacing=2 %}
      {% frame title="Sales by Category (Bar)" border_style="single" width=38 height=10 %}
      ...
      {% frame title="Monthly Revenue (Column)" border_style="single" width=38 height=10 %}
    ...
    {% hstack spacing=2 %}
      {% frame title="Trend (Line Chart)" border_style="single" width=38 height=10 %}
        {% linechart id="trend_line" data=state.trend_data width=34 height=6
           style="line" show_axis=true show_labels=false %}
        {% endlinechart %}
      {% endframe %}

      {% frame title="Activity HeatMap" border_style="single" width=38 height=10 %}
